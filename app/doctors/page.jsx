        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
          ...
          <div className="mt-6">
            <Tabs defaultValue="all" value={activeSpecialty} onValueChange={setActiveSpecialty}>
              <TabsList className="w-full flex overflow-x-auto pb-2 space-x-2 bg-transparent">
                {specialties.map((specialty) => (
                  <TabsTrigger
                    key={specialty.id}
                    value={specialty.id}
                    className="px-4 py-2 rounded-full data-[state=active]:bg-purple-500 data-[state=active]:text-white"
                  >
                    {specialty.name}
                  </TabsTrigger>
                ))}
